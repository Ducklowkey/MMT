...
void handle_client_message(Server* server, int client_index, char* buffer) {
   ...
   if (strncmp(buffer, "CREATE_ROOM", 11) == 0) {
...
else if (strcmp(buffer, "START_PRACTICE") == 0) {
        printf("Received START_PRACTICE message. Starting practice mode...\n");

        // Biến lưu trữ tạm thời dữ liệu nhận được
        char temp_buffer[BUFFER_SIZE];
        memset(temp_buffer, 0, BUFFER_SIZE);

        // Nhận dữ liệu từ client
        int bytes_read = read(client->fd, temp_buffer, sizeof(temp_buffer) - 1);
        if (bytes_read <= 0) {
            printf("Failed to receive practice config from client.\n");
            return;
        }
        temp_buffer[bytes_read] = '\0'; // Kết thúc chuỗi

        // Ghép nối dữ liệu vào buffer chính
        strcat(buffer, temp_buffer);
        printf("%s\n", buffer);
        // Tìm thông điệp SET_FORMAT
        if (strncmp(buffer, "START_PRACTICE-SET_FORMAT: ", 27) == 0) {
            const char* format_part = buffer + 27; // Bỏ qua "SET_FORMAT: "
            int num_questions, time_limit, num_easy, num_medium, num_hard;
            char subjects[256];

            // Phân tích cấu hình
            if (sscanf(format_part, "%d,%d,%d,%d,%d,%255[^\n]",
                    &num_questions, &time_limit, &num_easy, &num_medium, &num_hard, subjects) == 6) {
                //start_practice_mode(num_questions, time_limit, num_easy, num_medium, num_hard, subjects,client->fd);
                start_practice_mode(num_questions, time_limit, num_easy, num_medium, num_hard, subjects, client->fd);
      ...
      // Kiểm tra nếu nhận được thông điệp "LEAVE_PRACTICE", thoát khỏi chế độ luyện tập
    else if (strcmp(buffer, "LEAVE_PRACTICE") == 0) {
         printf("Client has left practice mode.\n");
    } 
        // Nếu thông điệp không hợp lệ (chưa xử lý)
}
